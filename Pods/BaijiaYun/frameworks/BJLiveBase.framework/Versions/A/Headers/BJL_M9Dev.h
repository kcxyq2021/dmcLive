...
static inline void bjl_dispatch_on(dispatch_queue_t queue, dispatch_block_t block) {
    if (dispatch_queue_get_label(DISPATCH_CURRENT_QUEUE_LABEL) == dispatch_queue_get_label(queue)) block();
    else dispatch_async(queue, block);
}
...
// rt: run-time
#define bjl_invoke_rt(OBJ, ...)                     { bjl_invoke_val_rt(void *, nil, OBJ, __VA_ARGS__); }
...
#define bjl_invoke_sel_iter(INDEX, VAR) \
    bjl_metamacro_if_eq(bjl_metamacro_is_even(INDEX), 1) \
    /* ? */ (VAR) \
    /* : */ ()
...
#define bjl_invoke_args_iter(INDEX, VAR) \
    bjl_metamacro_if_eq(0, bjl_metamacro_is_even(INDEX)) \
    /* ? */ ({ __typeof__(VAR) arg = VAR; [invocation setArgument:&arg atIndex:index++]; }) \
    /* : */ ()
...
@dynamic PROPERTY; \
- TYPE GETTER { id PROPERTY = objc_getAssociatedObject(self, @selector(PROPERTY)); return DECODE; } \
- (void)SETTER TYPE PROPERTY { objc_setAssociatedObject(self, @selector(PROPERTY), ENCODE, OBJC_ASSOCIATION_RETAIN_NONATOMIC); }

#define bjl_associate_reference_type(TYPE, PROPERTY, GETTER, SETTER, POLICY) \
@dynamic PROPERTY; \
- TYPE GETTER { return objc_getAssociatedObject(self, @selector(PROPERTY)); } \
- (void)SETTER TYPE PROPERTY { objc_setAssociatedObject(self, @selector(PROPERTY), PROPERTY, POLICY); }
